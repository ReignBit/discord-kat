...
class Kat(commands.Bot):
    ...
    def __init__(self, **options):
        ...
        self.is_launched_through_orwell = False # Set to True through command line, if set to True will not restart self, since orwell manages that.
        self._is_maintenance_mode = bool(self.settings['maintenance_mode'])  # Should we be in Maintenance mode?
        ...
        self.is_first_boot = 1  # flag used to check whether to load cogs. (is this the first boot or reconnect)
        self.is_restart_scheduled = 0   # set to 1 when we are going to restart Kat. Mostly used to stop presence update
        self._current_presence = None   # Current 'is playing...' message. Populated whenever we change presence
        self.guild_count = -1  # populated in on_ready()
        self.default_prefix = self.settings['default_prefix']    # If a guild does not have a custom prefix data, then their guild prefix will be set to this.
        self.event_manager = None # Instance of utilities.events.EventManager. Populated on_ready
    ...
    async def on_ready(self):
        ...
        self.log.info("- Can see {} users, {} guilds ".format(len(self.users), len(self.guilds)))
    ...
    def get_custom_prefix(self, bot, message):
        ...
        prefix = self.sql.ensure_exists("KatGuild", guild_id=message.guild.id).prefix
                ...
                if os.path.isdir(file_path + "/" + file) and "__" not in file:
                    ...
                    if "__" not in file or "logs" not in file and file.endswith(".py"):
                        try:
                            ...
                            lines += sum(1 for line in open(file_path + "/" + file
                                                            , encoding="utf-8"))
