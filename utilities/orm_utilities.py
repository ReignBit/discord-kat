...
class SqlEngine:
    ...
    def __init__(self, bot=None, local=True, use_kat_logger=True, standalone=False, options=None):
        ...
        if bot is not None:
            ...
            self._sql_engine = sqlalchemy.create_engine("mysql+mysqldb://{}:{}@{}/{}".format(_sql_settings['user'],
                                                                                    ...
                                                                                    _sql_settings['db']))
            if use_kat_logger: self.log.debug("Created BOT SQL Engine")
        elif not standalone:
    ...
    def sql_test(self):
        """ Simply outputs the length of KatGuilds"""
    ...
    def query(self, kat_class: str):
        ...
        with self.get_sql_session() as session:
            if kat_class is "KatGuild": return session.query(KatGuild)
            if kat_class is "KatMember": return session.query(KatMember)
            if kat_class is "KatUser": return session.query(KatUser)
        ...
        with self.get_sql_session() as session:
            ...
            for user in user_ids:
                if user not in members:
                    self.log.debug("Found unguilded user `%s`, deleting..." % user)
    ...
    def ensure_exists(self, kat_class, guild_id=None, user_id=None) -> Union[KatGuild, KatUser, KatMember]:
        ...
        if kat_class is "KatGuild":
            if guild_id is not None:
                with self.get_sql_session() as session:
                    result = session.query(KatGuild).filter_by(guild_id=guild_id).first()
                    if result == None:
            ...
            if guild_id is not None and user_id is not None:
                with self.get_sql_session() as session:
                    result = session.query(KatMember).filter_by(guild_id=guild_id, user_id=user_id).first()
                    if result == None:
            ...
            if user_id is not None:
                with self.get_sql_session() as session:
                    result = session.query(KatUser).filter_by(user_id=user_id).first()
                    if result == None:
...
if __name__ == "__main__":
    sql = SqlEngine(bot=None, local=False, use_kat_logger=False, standalone=True)
