...
class Translate(KatCog):
    def __init__(self, bot):
        ...
        self.lang_list = read_resource("languages.json")

    async def _translate(self, from_lang, to_lang, msg):
        self.tranlator = Translator(
            to_lang=to_lang, from_lang=from_lang, provider="mymemory"
        )
    ...
    async def on_message(self, message):
        ...
        if re.search("[\u3040-\u30ff]", message.content) is not None:
            ...
            embed.set_author(
                name="{} Auto Translate".format(message.author.display_name),
                icon_url=message.author.avatar_url,
            )
            embed.set_footer(
                text="⚠️| Powered by opensource translations. May not be accurate."
            )
    ...
    async def translatelast(self, ctx, user: discord.User, lang="en"):
        ...
        for message in msgs:
            if message.author == user:
                result = await self._translate(
                    from_lang, to_lang, message.clean_content
                )
                embed = discord.Embed(
                    colour=discord.Colour(0x2B2B2B), description=result
                )
                embed.set_author(
        ...
        embed = discord.Embed(
            colour=discord.Colour(0x2B2B2B),
            description="Could not find {}'s last message. Could be older than 100 messages!".format(
                user.display_name
            ),
        )
        await ctx.send(embed=embed)
    ...
    async def translatemsg(self, ctx, message_id: int, lang="en"):
        ...
        embed = discord.Embed(colour=discord.Colour(0x2B2B2B), description=result)
        embed.set_author(
            name="{}".format(msgs.author.name), icon_url=msgs.author.avatar_url
        )
        await ctx.send(embed=embed)

    @commands.command()
    async def translate(self, ctx, lang="en"):
        """Attempts to translate the last sent message.
        If no language code is given then defaults to English.
        """
        ...
        if len(msgs[1].embeds) == 0:
            ...
            async with ctx.channel.typing():
                ...
                result = await self._translate(
                    from_lang, to_lang, msgs[1].clean_content
                )
                ...
                embed = discord.Embed(
                    colour=discord.Colour(0x2B2B2B), description="{}".format(result)
                )
                embed.set_author(
                    name="{}".format(msgs[1].author.display_name),
                    icon_url=msgs[1].author.avatar_url,
                )
    ...
    async def translatetext(self, ctx, lang="en"):
        ...
        try:
            # self.log.debug(msgs)
            await self._translate(from_lang, to_lang, msg.clean_content)
        except Exception as e:
            self.log.debug(e)
            await ctx.send("Unable to find message. Could be deleted.")
